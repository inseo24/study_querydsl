...
//        QMember member = QMember.member;
        ...
        assertThat(findMember.getUsername()).isEqualTo("member1"); // 파라미터 바인딩 처리
    }
     ...
     * 외부 조인 불가능 -> join on 절을 사용하면 외부 연관 관계가 없는 테이블도 외부 조인 가능
     */
    @Test
...
//                .where(team.name.eq("teamA")) // inner join 인 경우 on절 없이 where 절 쓰기
                .fetch();
    ...
    public void fetchJoinNo() throws Exception {
        ...
        boolean loaded =
                emf.getPersistenceUnitUtil().isLoaded(findMember.getTeam()); // loading 된 엔티티인지 체크
